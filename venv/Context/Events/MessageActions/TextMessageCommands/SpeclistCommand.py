...
class SpeclistCommand(BaseCommandEvent):

    def __init__(self, session, longpool, args):
        super(SpeclistCommand, self).__init__(session, longpool, args)

    def onEventReceive(self, event):
        peer_id = event.object.message["peer_id"]
        adminLists = AdminRepository.getListByStatus(peer_id, AUTHORITY.SPECIAL_ADMIN)
